...
class Adder:
    ...
    """
    def __init__(self, num_bit, num_adder, clk_freq, tech, config, mapping):
        self.tech = tech
        self.config = config
    ...
    def calculate_area(self, new_height, new_width, option='NONE'):
        ...
        if new_height and option == 'NONE':
            ...
            height = new_height
            width = w_adder * h_adder * self.num_adder / new_height
        elif new_width and option == 'NONE':
            if w_adder > new_width:
                raise ValueError("Adder width exceeds assigned width.")
            width = new_width
            height = w_adder * h_adder * self.num_adder / new_width
        else:
            width = w_adder * self.num_adder
            height = h_adder
    ...
    def calculate_latency(self, cap_load, num_read):
        ...
        for stage in range(self.num_bit):
            # NAND NMOS pull-down
    ...
    def calculate_power(self, num_read, num_adder_per_op):
        vdd = self.vdd
        total_nand = 9 * self.num_bit * self.num_adder
        ...
        active_adders = min(num_adder_per_op, self.num_adder)

        energy_per_bit = (
            (cap_in * 6 + cap_out * 2) +       # First stage
            (cap_in * 7 + cap_out * 3) * (self.num_bit - 1) +  # Later stages
            (cap_out + cap_in) * 2 +           # #2 and #3
            (cap_out + cap_in * 2) +           # #4
            (cap_out + cap_in * 3) +           # #5
            (cap_out + cap_in) +               # #6
            (cap_out + cap_in * 3) * (self.num_bit - 1) +
            (cap_out + cap_in) * 3 * (self.num_bit - 1)  # #3, #6, #7
        ) * vdd * vdd

        self.read_energy = energy_per_bit * active_adders * num_read * delta
